...
describe("Tuples", function () {
  ...
  it("simple nested", function () {
    ...
    u = t, (1, 2, 3, 4, 5)\n\
    return u[0][1]";
    expect(util.run(code)).to.equal(54321);
  });
  ...
  it("singleton = 'hello',", function () {
    ...
    singleton = 'hi',\n\
    return singleton[0]";
    expect(util.run(code)).to.equal('hi');
  });
  ...
  it("a += (4, 5)", function () {
    ...
    a += (4, 5)\n\
    return a";
    expect(util.run(code)).to.deep.equal([1, 2, 3, 4, 5]);
  });

  it("(4, 5) * 2", function () {
    var code = "\
    return (4, 5) * 2";
    expect(util.run(code)).to.deep.equal([4, 5, 4, 5]);
  });
  ...
  it("(True,) *= 3", function () {
    ...
    a *= 3\n\
    return a";
    expect(util.run(code)).to.deep.equal([true, true, true]);
  });
  ...
  it("for ((i,(j, l)),k) in [[[1,[2, 7]],5], [[3,[4, 8]],6]]: ...", function () {
    ...
    for ((i,(j, l)),k) in [[[1,[2, 7]],5], [[3,[4, 8]],6]]:\n\
      total += i + j + k + l\n\
    return total";
    expect(util.run(code)).to.equal(36);
  });
